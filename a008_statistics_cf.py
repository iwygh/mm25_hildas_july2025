...
def delta_xyiW(a0,b0,siga,sigb,rhoab,probmass):
    ellipse_x_vec,ellipse_y_vec = ellipse_points_2(a0,b0,siga,sigb,rhoab,probmass)
    ...
    return delta_x_deg,delta_y_deg,delta_i_deg,delta_W_deg
#%%
import numpy as np
import pandas as pd
...
for iseed in range(nseeds):
    ...
    ideg_mat = np.loadtxt('b004_'+seed+'_ideg_clones_'+yrsstr+'.csv',delimiter=',')
    Wdeg_mat = np.loadtxt('b004_'+seed+'_nodedeg_clones_'+yrsstr+'.csv',delimiter=',')
    ...
    for it in range(nt-1):
        ...
        rhoab = rhoabvec[it+1]
        p0 = probmasses[0]
        delta_x_deg,delta_y_deg,delta_i_deg,delta_W_deg = delta_xyiW(a0,b0,siga,sigb,rhoab,p0)
        sx1vec.append(delta_x_deg)
        sy1vec.append(delta_y_deg)
        si1vec.append(delta_i_deg)
        sW1vec.append(delta_W_deg)
        p1 = probmasses[1]
        delta_x_deg,delta_y_deg,delta_i_deg,delta_W_deg = delta_xyiW(a0,b0,siga,sigb,rhoab,p1)
        sx2vec.append(delta_x_deg)
        sy2vec.append(delta_y_deg)
        si2vec.append(delta_i_deg)
        sW2vec.append(delta_W_deg)
        p2 = probmasses[2]
        delta_x_deg,delta_y_deg,delta_i_deg,delta_W_deg = delta_xyiW(a0,b0,siga,sigb,rhoab,p2)
